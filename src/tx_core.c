...
int tx_core(struct tx_core_config *config)
{
    ...
    for (;;) {
        ...
        uint16_t nb_rx = rte_ring_dequeue_burst(config->ring, (void **)(bufs + off), config->burst_size - off, NULL);
        off += nb_rx;

        if (unlikely(off == 0)) { // tx_ring is empty
            if (rte_atomic32_read(&global_loader_counter) == 0) { // loaders finished
